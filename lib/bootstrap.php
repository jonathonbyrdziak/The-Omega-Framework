...
if (!function_exists("omega_get_show_view")):
	/**
	 * Controller.
	 * 
	 ...
	 * @param string $name
	 */
	function omega_get_show_view( $name = null )
	{
		...
		if (!($view = omega_find(array($theme), "views".DS.$name.".php")))
		{
			$view = omega_find($paths, "views".DS.$name.".php");
		}
		if (!($model = omega_find(array($theme), "models".DS.$name.".php")))
		{
			$model = omega_find($paths, "models".DS.$name.".php");
		}
...
if (!function_exists("omega_show_view")):
	/**
	 * Function prints out the omega_get_show_view()
	 * 
	 * @param string $name
	 * @see omega_get_show_view
	 */
	function omega_show_view( $name = null )
	{
		echo omega_get_show_view($name);
	}
endif;

if (!function_exists("omega_show_ajax")):
	...
	function omega_show_ajax() 
	{
		...
		$html = omega_get_show_view( $_REQUEST['view'] );
...
if (!function_exists("omega_set_controller_path")):
	/**
	 * Function prints out the omega_get_show_view()
	 * 
	 * @param string $name
	 * @see omega_get_show_view
	 */
	function omega_set_controller_path( $name = null )
	{
...
if (!function_exists("get_theme_path")):
	 ...
	 * @since	1.5
	 */
	function omega_find($paths, $file)
	{
...
if (!class_exists("Multiple_Widget_Master")):
	...
	{
		$this->widget = apply_filters('omega_widget_setup', $widget);
		$widget_ops = array(
	...
	function widget($args, $instance)
	{
		...
		$show_view = apply_filters('omega_widget_view', $this->widget['show_view'], $widget, $instance, $args);
		echo omega_get_show_view($show_view, $args);
	}
	...
	function form($instance)
	{
		...
		foreach ($this->widget['fields'] as $field)
		{
			...
			{
				case 'text':
					echo '<input type="text" name="', $this->get_field_name($field['id']), '" id="', $this->get_field_id($field['id']), '" value="', $meta ? $meta : $field['std'], '" class="omega_text" />', 
					'<br/><span class="description">', $field['desc'], '</span>';
					break;
				case 'textarea':
					echo '<textarea class="omega_textarea" name="', $this->get_field_name($field['id']), '" id="', $this->get_field_id($field['id']), '" cols="60" rows="4" style="width:97%">', $meta ? $meta : $field['std'], '</textarea>', 
					'<br/><span class="description">', $field['desc'], '</span>';
					break;
				case 'select':
					echo '<select class="omega_select" name="', $this->get_field_name($field['id']), '" id="', $this->get_field_id($field['id']), '">';
				...
				case 'radio':
					foreach ($field['options'] as $option)
					{
						echo '<input class="omega_radio" type="radio" name="', $this->get_field_name($field['id']), '" value="', $option['value'], '"', $meta == $option['value'] ? ' checked="checked"' : '', ' />', 
						$option['name'];
					}
				...
				case 'checkbox':
					echo '<input type="hidden" name="', $this->get_field_name($field['id']), '" id="', $this->get_field_id($field['id']), '" /> ', 
						 '<input class="omega_checkbox" type="checkbox" name="', $this->get_field_name($field['id']), '" id="', $this->get_field_id($field['id']), '"', $meta ? ' checked="checked"' : '', ' /> ', 
					'<br/><span class="description">', $field['desc'], '</span>';
					break;
				case 'custom':
					echo $field['std'];
...
if (!function_exists('read_520_rss')):
	function read_520_rss()
	{
		...
		$msgs = get_option('omega_hide_messages',array());
		...
		foreach ($xml->channel->item as $item)
		{
			...
			{
				omega_message($item->title.'</p><p>'.$item->description, $id);
			}
			if ($item->category == 'Notifications')
			{
				omega_notification($item->title.'</p><p>'.$item->description, $id);
			}
		}
	}
endif;
 ...
 *
 */
function omega_notification( $message, $id )
{
	echo '<div id="message" class="message'.$id.' updated below-h2">'
	.'<a href="javascript:omega_hide_messages(\''.$id.'\');return false;" class="omega_checkmark"></a>'
	.'<p>'.$message.'</p></div>';
}
 ...
 *
 */
function omega_message( $message, $id )
{
